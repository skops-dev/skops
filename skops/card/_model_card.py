...
class Card:
    ...
    model_diagram: bool or str, default=True
        ...
        string to ``model_diagram`` will instead use that string as the section
        name for the diagram.

        If using a non-skops template, the model diagram cannot be added
        automatically unless a string is passed. It can, however, always be
        added later using :meth:`Card.add_model_plot`.
    ...
    def _populate_template(self, model_diagram: bool | str):
        """If initialized with a template, use it to populate the card.

        Parameters
        ----------
        model_diagram: bool or str
        ...
        if self.template == Templates.skops.value:
            ...
            if model_diagram:
                if isinstance(model_diagram, str):
        ...
        if model_diagram and isinstance(model_diagram, str):
            self.add_model_plot(section=model_diagram)
